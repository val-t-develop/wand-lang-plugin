...
import com.valtdevelop.wandlangplugin.debug.WandCatchpointType;
import org.jetbrains.annotations.NotNull;

public class WandCatchpointHandler extends XBreakpointHandler<XBreakpoint<WandCatchpointProperties>> {
    private final DebugProcess myDebugProcess;

    public WandCatchpointHandler(DebugProcess debugProcess, final Class<? extends WandCatchpointType> typeClass) {
        super(typeClass);
        myDebugProcess = debugProcess;
    }

    @Override
    public void registerBreakpoint(@NotNull XBreakpoint<WandCatchpointProperties> breakpoint) {
        final BreakpointManager manager = myDebugProcess.getBreakpointManager();
        try {
            manager.addWandCatchpoint(breakpoint);
     ...
     */
    @Override
    public void unregisterBreakpoint(@NotNull XBreakpoint<WandCatchpointProperties> key, final boolean temporary) {
        final BreakpointManager manager = myDebugProcess.getBreakpointManager();
        try {
